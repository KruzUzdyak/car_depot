package com.epam.volodko.controller.constant;

public final class ParameterName {

    public static final String ERROR_MESSAGE = "error_message";
    public static final String GREETING_MESSAGE = "greeting_message";
    public static final String MESSAGE = "message";
    public static final String LAST_REQUEST = "last_request";

    public static final String LOCALE = "locale";

    public static final String USER = "user";
    public static final String USER_ID = "user_id";
    public static final String USER_LOGIN = "user_login";
    public static final String USER_PASS = "user_password";
    public static final String USER_PASS_REPEAT = "user_pass_repeat";
    public static final String USER_NAME = "user_name";
    public static final String USER_PHONE = "user_phone";
    public static final String USER_ROLE = "user_role";
    public static final String ADMIN_WORKS_SINCE = "admin_works_since";
    public static final String ADMIN_NOTE = "admin_note";
    public static final String CLIENT_COMPANY = "client_company";
    public static final String CLIENT_NOTE = "client_note";

    public static final String LICENSE_TYPE = "license_type";
    public static final String LICENSE_TYPE_ID = "license_type_id";
    public static final String LICENSE_OBTAINING_DATE = "license_obtaining_date";
    public static final String LICENSE_NUMBER = "license_number";

    public static final String USER_UPDATE_LOGIN = "user_update_login";
    public static final String USER_UPDATE_PASS = "user_update_pass";
    public static final String USER_UPDATE_INFO = "user_update_info";

    public static final String CAR_LIST_TYPE = "car_list_type";
    public static final String CAR_LIST_ALL = "car_list_all";
    public static final String CAR_LIST = "car_list";

    public static final String ORDER = "order";
    public static final String ORDER_ID = "order_id";

    public static final String ORDER_LIST_TYPE = "order_list_type";
    public static final String ORDER_LIST_ALL = "order_list_all";
    public static final String ORDER_LIST_CLIENT = "order_list_client";
    public static final String ORDER_LIST_ADMIN = "order_list_admin";
    public static final String ORDER_LIST_CAR = "order_list_car";
    public static final String ORDER_LIST_ENTITY_ID = "order_list_entity_id";
    public static final String ORDER_LIST = "order_list";

}
